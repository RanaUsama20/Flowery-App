...
class CartRemoteDataSourceImpl implements CartRemoteDataSource {
  ...
  Future<CartModelEntity> getProductToCart() async {
    ...
    final response =
        await _cartRetrofitClient.getProductToCart('Bearer $token');
  ...
  @override
  Future<CartModelEntity> updateProductQuantity(
      String productId, int quantity) async {
    var token = await SharedPreferencesUtils.getString(AppValues.token);
    final response = await _cartRetrofitClient.updateProductQuantity(
        'Bearer $token', productId, {"quantity": quantity});
    return response.toEntity();
  }
  ...
  Future<CartModelEntity> deleteProductToCart(String cartItemId) async {
    var token = await SharedPreferencesUtils.getString(AppValues.token);
    final response = await _cartRetrofitClient.deleteProductToCart(
        'Bearer $token', cartItemId);
    return response.toEntity();
  }
